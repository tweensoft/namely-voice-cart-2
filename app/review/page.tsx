  ...
  async function automateCart() {
    try {
      ...
      const items = matches.map((match) => ({
        name: match.suggestedProductName || match.requestedName,
    ...
    >
      <section style={{ display: "grid", gap: 8 }}>
        ...
        <p style={{ margin: 0, color: "#475569", lineHeight: 1.6 }}>
          Her ser du forslag til nemlig.com. Varer med lav sikkerhed bør du åbne
          manuelt og tjekke først.
        </p>
      </section>
      ...
      >
        <div style={{ color: "#334155" }}>
          {readyCount} af {matches.length} varer har medium eller høj sikkerhed.
        </div>

        {matches.map((match, index) => (
          <div
            key={`${match.requestedName}-${index}`}
            style={{
          ...
          >
            ...
            >
              <strong>
                {match.quantity} x {match.requestedName}
              </strong>
              <span
                style={{
                  color:
            ...
            {match.note && (
              <div style={{ color: "#64748b" }}>{match.note}</div>
            )}
      ...
      >
        <h2 style={{ margin: 0, fontSize: 22 }}>
          Optional: forsøg automatisk kurv
        </h2>

        <p style={{ margin: 0, color: "#475569", lineHeight: 1.6 }}>
          Denne del kræver live selectors og kan ændre sig, hvis nemlig.com
          ændrer layout. Brug helst en testkonto først.
        </p>

        <input
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          placeholder="Nemlig e-mail"
          style={inputStyle}
        />
        ...
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
          <button
            onClick={automateCart}
            disabled={!matches.length || !email || !password || busy}
            style={buttonPrimary}
          >
            {busy ? "Arbejder..." : "Forsøg automatisk kurv"}
          </button>

          <a href="/" style={buttonLink}>
            Tilbage
          </a>
        </div>
